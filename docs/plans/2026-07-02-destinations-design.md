...
**Date:** 2026-07-02 (rev 2, after design review) · **Status:** Draft — supersedes `docs/dungeon-key-system-design-doc.md` (generic idle-clicker doc, kept for provenance) and rev 1 of this file (which had a third "Encounters" mode, now removed).

**Open decisions are marked `[D#]` and collected in §9 for veto.**
...
Every key has **difficulty D ≥ 1**. Base keys drop at D1 (85%), D2 (13%), D3 (2%) `[D2]`. Difficulty drives the dungeon:

- Enemy HP/damage: `× (1 + 0.4 × (D − 1))` `[D3]`
- Curated-roll rarity floor: D1 → Uncommon, D2 → Rare, D3 → Epic, D4+ → Epic floor + doubled Legendary weight
- Base curated rolls: `2 + D` (capped 6)
- Rush duration: `150 s + 20 s × (D − 1)`

### Fusion — the gold sink

Players fuse two keys into one at a **large gold cost**. The result carries **all modifiers of both** and **sums their difficulty**:
...
- Rules: max one modifier of each kind per key (can't fuse two color keys) `[D4]`; max D is capped (proposed 6).
- Cost scales with ambition: `fusionBaseCost × questLevel × D_result²` `[D5]` — the "large amounts of gold" lever, quadratic so triple-fused precision hunts are late-game purchases.
...
### Discovery (unchanged from rev 1, confirmed)

Single fragment counter as built (`FragmentProgress`, 0.01/kill, offline 0.25 efficiency). Bar fills to `fragmentTarget` → a **named location** is discovered from `locations.json` (e.g. *Blackroot Castle*, *Bogun's Bog*), tier rolled 60/25/12/3 (Common/Rare/Epic/Legendary). One pending at a time; bar holds at full while pending. Pending lasts 24 h, then expires refunding 50% of fragments. **The run starts on activation, not discovery** — absence is never punished.
...
**One-time:** a completed location instance is gone. Templates return to the discovery pool but can't repeat back-to-back, and each template can carry multiple endings so a revisit months later isn't a rerun `[D9]`.
...
├── LocationTable.cs      loads locations.json (templates, endings, dominant stats)
├── LocationState.cs      pending/active lifecycle; pure functions over injected nowUnix
├── LocationSpec.cs       fixed wave list + par time + ending tree reference
...
- **`SaveData`**: serialized key list, pending/active location (unix timestamps), run counters, visited-template history. Fragment progress already saved.
...
- **Unlock gating:** Destinations unlock after the first Zone Boss `[D10]`.
...
- `LocationState`: discovery trigger, pending expiry + refund, one-time template history — injected `nowUnix`.
...
2. **Sprint D2 — Locations:** fragment consumer, locations.json + templates, fixed-length runs, DialogueTree + ending outcomes (all six types), location UI + dialogue sheet.
3. **Sprint D3 — Content + polish:** more location templates/endings, archetype/color key art, balance pass with sim branch, fanfare/juice.

---

## 9. Open decisions (assumed, flagged for veto)

| # | Decision taken | Alternatives |
|---|---|---|
| D1 | 8 color families (Red, Orange, Brown, Yellow, Green, Blue, Purple, Neutral) | Fewer/more buckets; curated named palettes instead of HSV bucketing |
| D2 | Base keys drop at D1 85 / D2 13 / D3 2 | Difficulty scales with quest level at drop |
| D3 | Enemy mult +0.4/D; floors D1 Unc / D2 Rare / D3 Epic / D4+ Epic+2×Leg | Steeper/flatter curves |
| D4 | Fusion: max one modifier per kind; D cap 6 | Allow stacking same-kind (e.g. two colors → either) |
| D5 | Fusion cost = base × questLevel × D² | Linear cost; flat tiers |
| D6 | Excess keys sellable for gold | Keys not sellable; auto-convert over cap |
| D7 | Recruit with full roster: no cap exists → always joins; if a cap is added, fall back to Gold | Mailbox-style character queue |
| D8 | Dialogue choices select *which* reward — no empty-handed outcomes | Risk/reward choices that can whiff (louder, meaner) |
| D9 | Completed template returns to pool, no back-to-back repeats, multi-ending templates | Hard one-time-ever templates (content burn risk) |
| D10 | Unlock after first Zone Boss | From start; staggered per mode |

**Resolved in review (2026-07-02):** Encounters removed as a mode (par-bonus mechanic retained in both remaining modes) · key color = literal loot color, not stat · key kinds = color/type/class · class keys = 15 two-stat archetypes · fusion with summed difficulty · dungeons = timed wave rush, no fixed floors, no guaranteed boss · locations = one-time, fixed-length, dialogue-tree finale.
