...
```
┌─────────────────────────────────────────┐
│  💰 1.2M Gold    ⭐ Lv 34   🧩 ▓▓▓░ 7/12│  ← TopBar (160px)
│  ─── Wave 3/5 ─── Quest Lv 34 ───      │  ← WaveInfo (sub-row)
├─────────────────────────────────────────┤
...
│                                         │
│  👤 👤           ▓▓ ▓▓ ▓▓             │  ← Party (staggered) + Enemy silhouettes
│    👤 👤                               │
│                                         │
│ 🌿🌿🌿 foreground grass 🌿🌿🌿🌿🌿🌿 │  ← FG Layer (scroll 50px/s)
├─────────────────────────────────────────┤
│  [Verb 1]  [Verb 2]  [Verb 3]          │  ← VerbBar row 1 (240px total)
│  [Verb 4]  [Verb 5]                    │  ← VerbBar row 2
├─────────────────────────────────────────┤
│  ⚔️  |  📜  |  🎒  |  👥  |  🏪       │  ← BottomNav (120px)
└─────────────────────────────────────────┘
```
...
- TopBar + WaveInfo: 160px
- CombatArea: ~1400px (flexible fill)
- VerbBar: 240px (2 rows)
- BottomNav: 120px
...
- **Back row** (2 chars): higher position, smaller (~180x180), 80% scale
- **Front row** (2 chars): lower position, larger (~220x220), 100% scale
...
### Enemy Silhouettes
Three dark rounded rectangle Images on the right side of the combat area. Simple placeholder shapes (~120x180 each). No sprite loading.
...
- Verb cards: 5 cards with sample names and colors
...
└── Canvas (CanvasScaler 1080x1920)
    ├── TopBarPanel (anchor top, height 160)
    │   ├── TopRow (HorizontalLayoutGroup)
    │   │   ├── GoldLabel (TMP_Text)
    │   │   ├── LevelLabel (TMP_Text)
    │   │   └── FragmentBarGroup
    │   │       ├── FragmentBarBG (Image)
    │   │       ├── FragmentBarFill (Image, Filled)
    │   │       └── FragmentLabel (TMP_Text)
    │   └── WaveRow (HorizontalLayoutGroup)
    │       ├── WaveLabel (TMP_Text)
    │       └── QuestLevelLabel (TMP_Text)
    ...
    │   │   ├── PartySlot_BackLeft (RawImage 180x180)
    │   │   ├── PartySlot_BackRight (RawImage 180x180)
    │   │   ├── PartySlot_FrontLeft (RawImage 220x220)
    │   │   └── PartySlot_FrontRight (RawImage 220x220)
    │   ├── EnemyContainer (anchor right, width ~35%)
    │   │   ├── EnemySilhouette_0 (Image 120x180)
    │   │   ├── EnemySilhouette_1 (Image 120x180)
    │   │   └── EnemySilhouette_2 (Image 120x180)
    │   └── FG_Grass (RawImage, anchor bottom, h=280, ParallaxLayer speed=50)
    ├── VerbBarPanel (anchor bottom above nav, height 240)
    │   └── VerbGrid (GridLayoutGroup, 3 cols, 2 rows)
    │       ├── VerbCard_0..4 (Image bg + TMP name + TMP cooldown)
    ├── BottomNavPanel (anchor bottom, height 120)
    │   └── NavLayout (HorizontalLayoutGroup)
...
| `Assets/Scripts/UI/TopBarDisplay.cs` | Starquill.UI | MonoBehaviour: gold, level, fragments, wave info |
| `Assets/Scripts/UI/VerbBarDisplay.cs` | Starquill.UI | MonoBehaviour: verb card grid with placeholders |
...
| **Foreground Layer** |||||
| 4 | `fg_grass.png` | Grass, flowers, small debris. Fastest scroll. Overlaps bottom of characters. | 2160 x 280 | PNG, RGBA | Tiles horizontally. Mostly transparent with elements along the bottom. |
| **UI Elements** |||||
| 5 | `verb_card_bg.png` | Verb card background. Rounded rectangle with subtle border. | 300 x 100 | PNG, RGBA | 9-sliced. Tinted per verb stat type at runtime. |
