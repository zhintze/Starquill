...
namespace Crosstales.TrueRandom.EditorTask
{
   ...
   public class Launch : AssetPostprocessor
   {
      ...
      {
         if (importedAssets.Any(str => str.Contains(Crosstales.TrueRandom.EditorUtil.EditorConstants.ASSET_UID.ToString())))
         {
            Crosstales.Common.EditorTask.SetupResources.Setup();
            SetupResources.Setup();

            Crosstales.TrueRandom.EditorIntegration.ConfigWindow.ShowWindow(4);
         }
      }
   }
}
